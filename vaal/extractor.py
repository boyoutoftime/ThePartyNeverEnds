...
(
    ...
    \s*(=|≈|∝)\s*                     # Operador permitido (=, ≈, ∝)
    (                               
        [-+*/^A-Za-z0-9().±×eE,−∞^°′″]+  # Parte derecha con números, símbolos y letras
    )
)
(?=                                  # Lookahead para que termine bien
    (?:\s|$|[.,;:]|(?=[A-Z]))        # Finaliza antes de espacio, fin línea, puntuación o mayúscula (inicio frase)
)
"""
